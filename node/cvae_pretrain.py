...
def generated_generator(args, device, adj, features, labels, features_normalized, adj_normalized, idx_train, batch_seiz, diff):
    ...
    cvae_dataset_dataloader = DataLoader(cvae_dataset, sampler=cvae_dataset_sampler, batch_size=batch_seiz)
    ...
    cvae_features = cvae_features.to(args.device)
    ...
    for _ in trange(args.pretrain_epochs, desc='Run CVAE Train'):
        for _, (x, c) in enumerate(tqdm(cvae_dataset_dataloader)):
            ...
            t += 1
            print("U Score: ", U_score.data, " Best Score: ", best_score)
            if U_score > best_score:
                ...
                if t > args.warmup:
                    ...
                    for i in range(args.update_epochs):
                        ...
                        model_optimizer.step()
        if diff is not None:
            torch.save(cvae, open("{}/model_cvae/{}_g.pkl".format(exc_path, args.dataset), "wb"))
        else:
            torch.save(cvae, open("{}/model_cvae/{}_l.pkl".format(exc_path, args.dataset), "wb"))
